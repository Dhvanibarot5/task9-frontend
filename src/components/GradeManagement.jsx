...
function GradeManagement() {
  ...
  const loadData = () => {
    ...
    console.log("Loaded students:", savedStudents);
  ...
  const handleSubmit = (e) => {
    ...
    let updatedGrades;
    if (isEditing) {
      updatedGrades = grades.map((grade) => (grade.id === formData.id ? newGrade : grade));
    } else {
  ...
  const getStudentName = (studentId) => {
    ...
    console.log(`Found student: ${student ? student.name : "Unknown Student"}`);
  ...
  const getCourseName = (courseId) => {
    ...
    console.log(`Found course: ${course ? course.title : "Unknown Course"}`);
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredGrades.map((grade) => (
              <tr key={grade.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap font-semibold">{grade.grade}%</td>
                <td className="px-6 py-4 whitespace-nowrap">{grade.remarks}</td>
                <td className="px-6 py-4 whitespace-nowrap">{new Date(grade.submissionDate).toLocaleDateString()}</td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <button onClick={() => handleEdit(grade)} className="text-blue-600 hover:text-blue-900 mr-3">
                    <FaEdit />
                  </button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full">
            <h2 className="text-xl font-bold mb-4">{isEditing ? "Edit Grade" : "Add New Grade"}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end space-x-3 pt-4">
                <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 border rounded-md text-gray-600 hover:bg-gray-50">
                  Cancel
                </button>
